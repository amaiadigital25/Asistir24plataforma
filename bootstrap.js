...
function loadServerWithRuntimeFixes() {
  ...
  if (start !== -1 && end !== -1 && end > start) {
    const replacement = `async function quoteFromMail(parsed, gmailMeta) {
  ...
  const q = {
    ...
    tramos: { baseOrigen: null, origenDestino: null, destinoBase: null },
    kmTotal: null,
  ...
  try {
    ...
    q.kmTotal = kmTotal;
    q.subtotalKm = Math.round(kmTotal * tarifa.km);
    q.total = Math.round(tarifa.movida + q.subtotalKm);
